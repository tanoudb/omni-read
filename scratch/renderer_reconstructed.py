...
class TextRenderer:
    ...
    def _smooth_fill(
        ...
        try:
            ...
            band_k = cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (int(2 * sigma) | 1, int(2 * sigma) | 1),
            )
    ...
    def _draw_block(
        ...
        if top_aligned:
            # En mode verrouillé (top_aligned forcé), ix1/iy1 est la boîte d'ancrage
            # exacte sans marge. Ajouter du padding la décale vers le bas et la droite.
            left = ix1
            top = iy1
        else:
