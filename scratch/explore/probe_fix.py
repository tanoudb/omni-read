...
ZONES=[
 ("archmage-curriculum","p01",11,"archmage_out_text_MUSTfire"),
 ("30-years-have-passed-since-the-prologue","p01",3,"z30_3_bulle_NOfire"),
 ("30-years-have-passed-since-the-prologue","p01",1,"z30_1_bulle_NOfire"),
]
ube=TextRenderer._uniform_bg_erase
def spy(crop,mask,lb=None,grow=4,region_box=None,class_name=""):
    res=ube(crop,mask,lb,grow,region_box,class_name)
    print("   UBE fired=%s class=%s"%(res is not None,class_name))
    return res
TextRenderer._uniform_bg_erase=staticmethod(spy)
r=TextRenderer(); os.makedirs('scratch/explore/fix_out',exist_ok=True)
for serie,page,idx,tag in ZONES:
    ...
    cv2.imwrite('scratch/explore/fix_out/%s.png'%tag,out[py1:py2,px1:px2])
print("done")
