...
def run(image_path: Path, out_dir: Path, margin: int = 10):
    ...
    # ── Traduction (API Gemini) ──
    from core.translator_gemini import GeminiTranslator
    translator = GeminiTranslator(series_name="path-of-vengeance")
    
    texts_to_translate = [d.text_original or "" for d in keep]
    classes = [d.class_name for d in keep]
    
    try:
        translated_map = translator.translate_page_json(image_path.stem, texts_to_translate, classes)
    except Exception as e:
        print(f"Erreur de traduction API : {e}")
        translated_map = {}

    meta = []
    for i, d in enumerate(keep):
        try:
            d.text_translated = translated_map.get(str(i), str(d.text_original))
